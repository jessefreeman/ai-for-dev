...
- [[summary-nate-b-jones-conway-leak|I Analyzed 512,000 Lines of Leaked Code]] — Second deep-dive on the Claude Code leak. Surfaces [[conway|Conway]] (Anthropic's unannounced always-on agent) and frames it as the capstone of a 90-day platform play (Microsoft speedrun). Anchors the new [[behavioral-lock-in]] concept page — the load-bearing claim that Conway-class agents introduce a kind of vendor lock-in (the accumulated model of how you work) with no legal, regulatory, or technical portability framework.
...
- **[[behavioral-lock-in|Behavioral lock-in]]**: Conway-class always-on agents introduce a new lock-in surface. Files (Microsoft), records (Salesforce), and comms history (Slack) are painful but exportable. The accumulated model of how you work is not. *"There's no CSV of how this person thinks."* The policies around behavioral-context portability should ship before [[conway|Conway]] does, not after.
- **Three eras of AI competition**: Era 1 = models (2023–24, foundation-model benchmarks). Era 2 = interfaces (2024–25, harness wars: Claude Code vs Cursor vs OpenClaw). Era 3 = persistence and memory (2026, the always-on layer). All three frontier labs have converged on the same insight — the model is a loss leader; the persistent agent layer is the money product.
- **The four-step Anthropic playbook** (from the OpenClaw timeline): Copy the community-built primitive → subsidize the first-party version → block third-party access → ship a proprietary extension format. Visible across OpenClaw → Claude Code channels → [[managed-agents|Managed Agents]] → [[conway|Conway]].
